...
function wpmu_current_site() {
	...
	// Still no dice.
	if ( 1 == count( $sites ) )
		wp_die( sprintf( /*WP_I18N_BLOG_DOESNT_EXIST*/'A honlap nem létezik. Próbálja meg: <a href="%s">%s</a>.'/*/WP_I18N_BLOG_DOESNT_EXIST*/, $sites[0]->domain . $sites[0]->path ) );
	else
		wp_die( /*WP_I18N_NO_SITE_DEFINED*/'Nem található ilyen oldal. Ha mi vagyunk az oldal tulajdonosai segítséget találhatunk a <a href="http://codex.wordpress.org/Debugging_WPMU">WPMU hibakeresés</a> oldalon.'/*/WP_I18N_NO_SITE_DEFINED*/ );
}
...
function ms_not_installed() {
	...
	$title = /*WP_I18N_FATAL_ERROR*/'Hiba az adatbázis-kapcsolat létesítése során!'/*/WP_I18N_FATAL_ERROR*/;
	$msg  = '<h1>' . $title . '</h1>';
	if ( ! is_admin() )
		die( $msg );
	$msg .= '<p>' . /*WP_I18N_CONTACT_OWNER*/'Amennyiben a honlap nem jelenik meg, értesíteni kell a hálózat tulajdonosát.'/*/WP_I18N_CONTACT_OWNER*/ . '';
	$msg .= ' ' . /*WP_I18N_CHECK_MYSQL*/'Ha, te vagy a hálózat tulajdonosa, kérlek ellenőrizd, hogy ellenőrizd hogy a MySQL helyesen fut-e, hogy minden tábla hiba mentes.'/*/WP_I18N_CHECK_MYSQL*/ . '</p>';
	if ( false && !$wpdb->get_var( "SHOW TABLES LIKE '$wpdb->site'" ) )
		$msg .= '<p>' . sprintf( /*WP_I18N_TABLES_MISSING_LONG*/'<strong>Hiányzó adatbázis táblák.</strong> Ezt azt jelenti, hogy a MySQL nem fut, vagya  WordPress nincs helyesen telepítve, vagy valaki törölte a(z) <code>%s</code> állományt. Igazán bele kellene nézni az adatbázisba...'/*/WP_I18N_TABLES_MISSING_LONG*/, $wpdb->site ) . '</p>';
	else
		$msg .= '<p>' . sprintf( /*WP_I18N_NO_SITE_FOUND*/'<strong>Nem található weboldal: <code>%1$s</code>.</strong> Ezt az adatbázis táblát kerestem: <code>%2$s</code>, ebben az adatbázisban <code>%3$s</code>. Ez helyes?'/*/WP_I18N_NO_SITE_FOUND*/, rtrim( $domain . $path, '/' ), $wpdb->blogs, DB_NAME ) . '</p>';
	$msg .= '<p><strong>' . /*WP_I18N_WHAT_DO_I_DO*/'Jelenlegi tevékenységem'/*/WP_I18N_WHAT_DO_I_DO*/ . '</strong> ';
	$msg .= /*WP_I18N_RTFM*/'<a target="_blank" href="http://codex.wordpress.org/Debugging_WPMU">Hibajelentés</a> oldalának olvasása. Az iránymutatások valemelyike lehetséges, hogy segítséget nyújt számunkra a megoldásban.'/*/WP_I18N_RTFM*/;
	$msg .= ' ' . /*WP_I18N_STUCK*/'Ha már beleakadtunk ebbe az üzenetbe, akkor ellenőizni kell az adatbázis tartalmát a következő táblákban:'/*/WP_I18N_STUCK*/ . '</p><ul>';
