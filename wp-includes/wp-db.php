...
class wpdb {
	...
	function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'Hibás adatbázis prefix'/*/WP_I18N_DB_BAD_PREFIX*/);
	...
	function select( $db, $dbh = null ) {
		...
		if ( !@mysql_select_db( $db, $dbh ) ) {
			$this->ready = false;
			$this->bail( sprintf( /*WP_I18N_DB_SELECT_DB*/'<h1>Nem sikerült kiválasztani az adatbázist</h1>
...
<p>Ha semmi nem segít <strong>fel kell venni a kapcsolatot a szolgáltatóval</strong>. Ha bármi más hiba áll elő, bizonyosan sgítséget lehet találni a <a href=\'http://forum.wpm.hu/\' target="_blank">WordPress Magyarország Fórum</a>án.</p>'/*/WP_I18N_DB_SELECT_DB*/, htmlspecialchars( $db, ENT_QUOTES ), htmlspecialchars( $this->dbuser, ENT_QUOTES ) ), 'db_select_fail' );
			return;
		}
	}
	...
	function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR_FULL*/'WordPress adatbázis hiba %1$s a lekérdezésben %2$s készítette %3$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller );
		else
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR*/'WordPress adatbázis hiba %1$s a lekérdezésben %2$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query );
...
<h1>Nem sikerült létrehozni az adatbázis kapcsolatot</h1>
...
'/*/WP_I18N_DB_CONN_ERROR*/, htmlspecialchars( $this->dbhost, ENT_QUOTES ) ), 'db_connect_fail' );
	...
	function get_row( $query = null, $output = OBJECT, $y = 0 ) {
		...
		if ( $output == OBJECT ) {
			...
			$this->print_error(/*WP_I18N_DB_GETROW_ERROR*/' $db->get_row(string query, output type, int offset) -- A kimenet típusának egyeznie kell ezek egyikével: OBJECT, ARRAY_A, ARRAY_N'/*/WP_I18N_DB_GETROW_ERROR*/);
		}
	}
